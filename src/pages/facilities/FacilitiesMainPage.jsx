...
const FacilityMainPage = () => {
  ...
  const [selectedFacility, setSelectedFacility] = useState(null); // 카드와 시설마커를 연결하기 위한 변수
    ...
    const fetchData = async () => {
      try {
        ...
        const formatted = facilities.map((item, index) => ({
          id: index + 1,
          title: (typeof item.title === "object" ? item.title["#text"] : item.title) || "이름 없음",
          address:(typeof item.venue  === "object" ? item.venue ["#text"] : item.venue ) || "주소 없음",
          phone: (typeof item.phone === "object" ? item.phone["#text"] : item.phone) || "전화번호 없음",
          status: "진료 중",
  ...
  return (
    <div className="mt-4" style={{ paddingTop: "160px" }}>
      ...
      {/* 지도 */}
      <FacilityMap facilities={posts} selectedFacility={selectedFacility}/>
